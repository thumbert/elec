  ...
  bool isScheduleValid() {
    ...
    for (var it in schedule) {
      if (it.interval is! Hour)
        throw new ArgumentError('Not an hourly schedule!');
    }
    date = new Date.fromTZDateTime(schedule.first.interval.start);
    var end = new Date.fromTZDateTime(schedule.last.interval.start);
    if (date != end)
      throw new ArgumentError('Virtual schedule extends more than one calendar day.');
...
_buildOneVirtual(xml.XmlBuilder builder, Virtual virtual) {
  builder.element('mes:DemandBid', nest: () {
    String type = virtual.virtualType == VirtualType.decrement ? 'Decrement' : 'Increment';
