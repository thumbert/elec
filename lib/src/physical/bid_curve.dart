library physical.bid_curve;

import 'dart:collection';

import 'price_quantity_pair.dart';

class BidCurve extends ListBase<PriceQuantityPair> {
  var _data = <PriceQuantityPair>[];

  BidCurve() : _data = <PriceQuantityPair>[];

  BidCurve.fromIterable(Iterable<PriceQuantityPair> xs) {
    for (var x in xs) {
      add(x);
    }
  }
