...
import 'package:elec/src/physical/virtual_transactions/lib_virtuals.dart';
import 'package:elec/src/physical/price_quantity_pair.dart';

void virtualTest() {
  var day = Date(2019, 5, 20, location: getLocation('America/New_York'));
  var hours = day
      ...
      .toList();
  var scheduleInc =
      TimeSeries.from(hours, hours.map((hour) => PriceQuantityPair(20, 75)));
  var inc = Virtual(scheduleInc, 4006, VirtualType.increment);
  var scheduleDec =
      TimeSeries.from(hours, hours.map((hour) => PriceQuantityPair(300, 75)));
